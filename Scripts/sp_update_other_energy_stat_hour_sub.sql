-- DROP PROCEDURE public.sp_update_other_energy_stat_hour_sub(text);

CREATE OR REPLACE PROCEDURE public.sp_update_other_energy_stat_hour_sub(IN p_device_id text)
 LANGUAGE plpgsql
AS $procedure$
  ...
  DECLARE 
    ...
    v_cursor CURSOR FOR 
      SELECT 
        ...
        hdsm.device_type,
        hdsm.key_id,
        SUM(hdsm.dbl_stats) AS dbl_stats,
      ...
      JOIN hd_config_param hcp ON hcp.factory_id = hd.factory_id -- 碳排係數表
  ...
  BEGIN
    ...
    RAISE NOTICE '準備新增非電儀表 % 的耗用量時統計資料...', p_device_id;
    ...
    LOOP
        ...
        INSERT INTO hd_device_statistics_hourly(stat_time, device_id, stat_type, device_type, key_id, dbl_stats, dbl_avg, dbl_min, dbl_max, kgco2e)
        VALUES (v_rec.truncated_stat_time, v_rec.device_id, v_rec.stat_type, v_rec.device_type, v_rec.key_id, 
                v_rec.dbl_stats, v_rec.dbl_avg, v_rec.dbl_min, v_rec.dbl_max, v_rec.kgco2e)
        ...
        IF is_last /*OR (v_rec.device_id <> v_nrec.device_id OR v_rec.key_id <> v_nrec.key_id)*/ THEN
            RAISE NOTICE '  id: %, time: % -> 最後一筆!', v_rec.device_id, v_rec.truncated_stat_time;
    ...
    EXCEPTION
          WHEN OTHERS THEN
            -- 捕捉異常並記錄錯誤，不中斷主迴圈
            RAISE EXCEPTION ' 處理設備 % 時發生錯誤，資料時間: %，原因: %。', p_device_id, v_rec.truncated_stat_time, SQLERRM;
