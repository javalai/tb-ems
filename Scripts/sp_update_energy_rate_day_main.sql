...
CREATE OR REPLACE PROCEDURE public.sp_update_energy_rate_day_main()
 LANGUAGE plpgsql
AS $procedure$
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE '開始處理 % 的瞬間量日統計資料...', v_device_id;

    BEGIN

      PERFORM DBLINK('host=/var/run/postgresql port=5432 user=postgres dbname=thingsboard',
        FORMAT('CALL public.sp_update_energy_rate_day_sub( ''%s'' )', v_device_id));
