-- DROP PROCEDURE public.sp_update_other_energy_rate_day_sub(text);

-- 物理量只須算到日統計以上
CREATE OR REPLACE PROCEDURE public.sp_update_other_energy_rate_day_sub(IN p_device_id text)
 LANGUAGE plpgsql
AS $procedure$
  ...
  DECLARE 
    ...
    v_cursor CURSOR FOR 
      SELECT 
        DATE_TRUNC('day', TO_TIMESTAMP(tk.ts/1000)) AS stat_time,
        hd.device_id, 
        3 AS stat_type, -- 日統計
        hd.device_type,
...
--        END, 0) AS dbl_stats,
  ...
  BEGIN
    ...
    LOOP
        ...
        INSERT INTO hd_device_statistics_daily(stat_time, device_id, stat_type, device_type, key_id, dbl_stats)
