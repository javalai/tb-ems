...
DECLARE 
  ...
  v_cursor CURSOR FOR 
    ...
    SELECT 
      ...
      hdsh.device_type,
      hdsh.key_id,
      SUM(hdsh.dbl_stats) AS dbl_stats,
      ...
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='PE' THEN hdsh.dbl_stats END) AS peak_energy,
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='PP' THEN hdsh.dbl_stats END) AS partial_peak_energy,
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='OP' THEN hdsh.dbl_stats END) AS off_peak_energy,
      SUM(hdsh.charge) AS charge,
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='PE' THEN hdsh.charge END) AS peak_charge,
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='PP' THEN hdsh.charge END) AS partial_peak_charge,
      SUM(CASE WHEN kd."key" = 'ConsumedEnergy' AND hdsh.period_type='OP' THEN hdsh.charge END) AS off_peak_charge,
      SUM(hdsh.dbl_stats)*hcp.float_value AS kgco2e
    FROM hd_device hd
    ...
    WHERE hd.device_type IN ('E', 'W', 'G', 'A', 'S')
      AND kd."key" IN ('ConsumedEnergy', 'water_consumed_volume', 'air_consumed_volume', 'gas_consumed_volume', 'steam_consumed_volume')
