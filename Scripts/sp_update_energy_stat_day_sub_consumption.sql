-- DROP PROCEDURE public.sp_update_energy_stat_day_sub_consumption(text);
...
CREATE OR REPLACE PROCEDURE public.sp_update_energy_stat_day_sub_consumption(IN p_device_id text)
 LANGUAGE plpgsql
AS $procedure$
...
DECLARE 
  ...
  v_cursor CURSOR FOR 
     SELECT 
      -- st.latest_stat_time,
      ec.bucket AS truncated_stat_day,
      hd.entity_id,
      hd.device_id, 
      3 AS stat_type, -- 日統計
      hd.device_type,
    ...
    JOIN fn_get_energy_consumption2(
    		hd.device_id,
    		DATE_TRUNC('day', COALESCE(st.latest_stat_time, TO_TIMESTAMP('1911-01-01', 'YYYY-MM-DD')::TIMESTAMP)),
    		NULL,
    		'DAY') ec 
      ON ec.device_id = hd.device_id
    WHERE hd.device_id = p_device_id
    GROUP BY 
      truncated_stat_day, hd.device_id, stat_type, hd.device_type, hkc.consumption_key_id, ec.energy_consumption, hcp.float_value
    ORDER BY
      truncated_stat_day, hd.device_id, stat_type, hd.device_type, hkc.consumption_key_id
    ;
...
BEGIN
  ...
  IF v_DEBUG THEN RAISE NOTICE 'v_cursor opened ...'; END IF;
  ...
  LOOP
      ...
      UPDATE hd_device_statistics_daily
         SET dbl_stats = v_record.dbl_stats,
             kgco2e = v_record.kgco2e
       WHERE stat_time = v_record.truncated_stat_day
         AND device_id = v_record.device_id
         AND stat_type = v_record.stat_type 
         AND device_type = v_record.device_type
         AND key_id = v_record.key_id
      ;
      ...
      IF is_last THEN
          ...
          IF v_DEBUG THEN RAISE NOTICE '    device_type: %', v_record.device_type;  END IF;
          IF v_DEBUG THEN RAISE NOTICE '    key_id: %', v_record.key_id;  END IF;
  ...
  RAISE NOTICE '  更新 % 的耗用量日統計資料，共更新 % 筆，計時 % 秒。', p_device_id, v_ins_rows, v_duration;
...
COMMENT ON PROCEDURE sp_update_energy_stat_day_sub_consumption(TEXT)
IS '電能日統計副程式
參數：
