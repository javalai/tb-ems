-- DROP PROCEDURE public.sp_update_other_energy_stat_hour_main();

CREATE OR REPLACE PROCEDURE public.sp_update_other_energy_stat_hour_main()
 LANGUAGE plpgsql
AS $procedure$
  ...
  BEGIN
    ...
    LOOP
      ...
      RAISE NOTICE '開始處理非電能源 % 的耗用量時統計資料...', v_device_id;
      ...
      BEGIN
        ...
        PERFORM DBLINK('host=/var/run/postgresql port=5432 user=postgres password=kenda2415@ dbname=thingsboard',
          FORMAT('CALL public.sp_update_other_energy_stat_hour_sub( ''%s'' )', v_device_id));

        SELECT EXTRACT(EPOCH FROM (CLOCK_TIMESTAMP()-v_start_time)) INTO v_duration;

        RAISE NOTICE '非電能源 % 的耗用量時統計資料已處理完成，計時 % 秒', v_device_id, v_duration;
    ...
    -- 捕捉異常並記錄錯誤，不中斷主迴圈
    RAISE NOTICE '所有非電能源的耗用量時統計處理完成。';
